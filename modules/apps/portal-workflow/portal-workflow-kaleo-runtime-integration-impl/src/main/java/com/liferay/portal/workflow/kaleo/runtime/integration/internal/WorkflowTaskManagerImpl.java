...
public class WorkflowTaskManagerImpl implements WorkflowTaskManager {
	...
	public List<User> getAssignableUsers(long companyId, long workflowTaskId)
		...
		return _getAllowedUsers(_ASSIGN_TASK_ACTION_TYPE, workflowTaskId);
	}
	...
	public List<User> getUsersThatCanSeeNotifications(
		...
		return _getAllowedUsers(_VIEW_NOTIFICATION_ACTION_TYPE, workflowTaskId);
	}

	private List<User> _getAllowedUsers(
		String actionType, long workflowTaskId) throws WorkflowException {
		...
		try {
			...
			if (kaleoTaskInstanceToken.isCompleted() &&
				Objects.equals(actionType, _ASSIGN_TASK_ACTION_TYPE)) {
	...
	private final String _VIEW_NOTIFICATION_ACTION_TYPE = "VIEW_NOTIFICATION";
	private final String _ASSIGN_TASK_ACTION_TYPE = "ASSIGN_TASK";

	private void _populateAllowedUsers(
		String actionType, Set<User> allowedUsers, long assignedUserId,
		KaleoTaskAssignment kaleoTaskAssignment,
		KaleoTaskInstanceToken kaleoTaskInstanceToken)
		throws PortalException {

		if (Objects.equals(
			kaleoTaskAssignment.getAssigneeClassName(), User.class.getName())) {
		...
		if ((role.getType() == RoleConstants.TYPE_DEPOT) ||
			...
			if (Objects.equals(
				...
				if (Objects.equals(actionType, _ASSIGN_TASK_ACTION_TYPE)) {
			...
			if (Objects.equals(actionType, _ASSIGN_TASK_ACTION_TYPE)) {
				ListUtil.filter(
					...
					user -> user.isActive() && user.getUserId() != assignedUserId);
			} else {
