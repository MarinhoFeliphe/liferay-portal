...
<%@ page import="com.liferay.notification.web.internal.foo.folder.FooDisplayContext" %><%@
...
<%
FooDisplayContext fooDisplayContext = (FooDisplayContext)request.getAttribute("displayContext");

ObjectRelationship objectRelationship = fooDisplayContext.getObjectRelationship();
%>
...
<aui:form action="<%= editObjectEntryRelatedModelActionURL %>" method="post" name="fm">
	...
	<aui:input name="objectEntryId" type="hidden" value="<%= fooDisplayContext.getObjectEntryId() %>" />
	<aui:input name="objectRelationshipPrimaryKey2" type="hidden" value="" />

	<frontend-data-set:headless-display
		apiURL="<%= fooDisplayContext.getAPIURL() %>"
		creationMenu="<%= fooDisplayContext.getCreationMenu() %>"
		formName="fm"
		id="my"
		style="fluid"
	/>
...
<c:if test="<%= !fooDisplayContext.isDefaultUser() %>">
		...
		const selectRelatedModelHandler = Liferay.on(
			...
			() => {
				Liferay.Util.openSelectionModal({
					...
					url:
						'<%= fooDisplayContext.getRelatedObjectEntryItemSelectorURL() %>',
				});
			}
		);
